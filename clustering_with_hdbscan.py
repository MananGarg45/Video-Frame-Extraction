...
class ImageSelector(object):
    ...
    def __filter_optimum_brightness_and_contrast_images__(self, input_img_files):
        ...
        # Returning only those images which have good brightness and contrast
...
#         Hdbascan = hdbscan.HDBSCAN(min_cluster_size=2, min_samples = 2, metric='manhattan').fit(all_hists)
        Hdbascan = hdbscan.HDBSCAN(min_cluster_size = 2, min_samples = 2, metric='manhattan').fit(all_dst)
        labels = np.add(Hdbascan.labels_,1)
#         labels = Hdbascan.labels_
        print(labels)
        print(len(labels))
        nb_clusters = len(np.unique(Hdbascan.labels_))
        print(np.unique(Hdbascan.labels_))
        ...
        print("files_clusters_index_array_of_only_one_image: ", files_clusters_index_array_of_only_one_image)
...
#     def __plots_for_clustering(self, Hdbascan, all_hists):
    ...
    # To Remove the Blurry Images from the final_images list

    def end_laplacian(self, fil, num):
        cnt = 0
        var_laps = []
        for image_i in range(0, num):
            img_file = fil[image_i]
            img = cv2.cvtColor(img_file, cv2.COLOR_BGR2GRAY)

            # Calculating the blurryness of image
            var_lap = self.__variance_of_laplacian__(img)
            
            if var_lap>400:
                var_laps.append(image_i)
            else:
                cnt+=1
        
        if (cnt/num)<=0.75:
            var_laps_array = np.array(var_laps)
            selected = [fil[i] for i in var_laps_array]
        else:
            return fil
    ...
    def __get_best_images_index_from_each_cluster__(
        ...
        for cluster_i in clusters:
            ...
            # Selecting image with low blurr(high laplacian) score
            try:
    ...
    # def select_best_frames(self, input_key_frames, number_of_frames):
        ...
        if len(input_key_frames) >= 1:
            ...
            sortedlist = np.sort(selected_images_index)
            print("Final List: ", sortedlist)
        ...
        # files_clusters_index_array_of_only_one_image = None
        for imag in files_clusters_index_array_of_only_one_image:
            # print(files_clusters_index_array_of_only_one_image) # This is printed as output in the cell
            path = output_folder+'/'+str(i)
            try:
                if not os.path.isdir(output_folder):
                    os.mkdir(output_folder)
            ...
            try:
                ...
                pass
            cv2.imwrite(os.path.join(path, str(imag)+'.jpeg'), input_key_frames[imag])
            i=i+1
